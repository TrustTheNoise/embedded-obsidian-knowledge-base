...
$y=f (x)$ - y is  equal to f of x
$x mapsto y$ - читается x mapped onto y, это значит, что для каждого элемента x из области arguments существует правило, по которому он преобразуется в элемент y из values of function
$x mapsto f (x)$ - то же самое, но только values of function записано как f(x)

Также функции записываются как
$$f: #text(fill: red)[$X$] -> #text(fill: yellow)[$Y$]$$
Читается как function f from set X to set Y. Это значит, что эта функция принимает arguments: $x in X$ и переводит в values $y in Y$.
...
Таким образом $x in RR$ но domain $D_(f) = RR and p o s i t i v e$.

Set $X$ называется set of departure(область отправления).
Set $Y$ называется codomain.
Range же называется subset для Y которые определены на функции для соответствующих x. Т.е.
$R_(f) subset Y and   R_(f)={y; y=f (x) " for some " x in D_(f) }$
Иногда ещё range записывается как $V_(f)$ т.к. это set of **V**alues of function.
...
$$x^(2)+y^(2)=r^(2)$$
$$F (x,y)=x^(2)+y^(2)-r^(2)$$
By this implicit function we can't easily understand dependency of y by argument x. As you remember this function is describe circle on the plane $R^2R^2$
...
Итак у нас есть функция $f: X -> Y$

Функция f является surjection если range этой функции определён на всём множестве Y. То есть если $R_(f)=Y$
![[Pasted image 20250703144532.png|300]]
Функция f является injection если $f (x_(1))=f (x_(2)) <=> x_(1)=x_(2)$ ещё такую функцию называют 1-to-1 
...
С surjection есть одна фишка. Ведь мы можем менять set Y(codomain) так, чтобы он был одинаковым с range. Просто считаем что $f:X -> R_(f)$.

# Inverse function

Если функция $f:X -> Y$ является bijection, то есть surjection и injection одновременно, то такой функции можно сделать inverse function $^(-1): Y -> X$
...
Ну чтобы можно было обе функции нарисовать на одной плоскости можно просто заменить в обратной функции y на x. Так чтобы $f^(-1) (y) -> f^(-1) (x)$
...
- $f:X -> Y$
- $g: Y ->$![[Pasted image 20250703153413.png|100]]
...
$$g circ f:
	X 
	xrightarrow #text(fill: lightgreen)[$f$]
	Y 
	xrightarrow #text(fill: aqua)[$g$] 
	Z$$
...
$$frac(d, d t) (g circ f) (t) = g'(f (t)) dot.op f'(t)$$